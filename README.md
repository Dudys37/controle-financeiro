...
23. 🔜 **Backend & melhorias futuras** — anexos com upload real (Firebase Storage), integrações de mercado/FIPE, Open Finance via provedor, Gmail/Calendar real com OAuth, B3 com proxy, notificações push, visões Kanban/calendário, e modularização gradual do `app.js` (que está grande). Com Patrimônio, **todas as áreas de vida planejadas estão cobertas** — finanças, trabalho, carreira, compras, decisões, metas, integrações e patrimônio.
